...
class isosurface:
    ...
    def field_density_cube_p(self,p,dbox,grids,ngrids,box,d,sigma,output=None):
        field  = np.zeros_like(self.field)
        for pos in p:
            indices = np.ceil(pos/dbox)    
            idx = self.ref_idx + indices
            idx %= ngrids
            idx = idx.astype(int)

            dr = abs(pos - grids[idx[:,0],idx[:,1],idx[:,2]])
    ...
    def field_density_cube(self,pos,d=np.array([1,1,1]),num_workers=None):
        """
        ...
        if num_workers is not None:
            process = []
            output = mp.Queue()
            pos_divided = np.array_split(pos,num_workers)
            for i in range(num_workers):
                p = mp.Process(target=self.field_density_cube_p,args=(pos_divided[i],dbox,grids,ngrids,box,d,sigma,output))
                process.append(p)
            ...
            for p in pos: 
                self.field += self.field_density_cube_p(p,dbox,grids,ngrids,box,d,sigma)
